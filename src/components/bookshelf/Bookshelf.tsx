...
export const Bookshelf: React.FC<BookshelfProps> = ({ books, onSelectBook, onAddBook, selectedBookId }) => {
    return (
        <div className="p-4 md:p-12 w-full max-w-4xl mx-auto">
            <div className="flex items-end justify-center gap-1 md:gap-2 perspective-1000 border-b-8 border-[#3d2b1f] shadow-[0_20px_50px_rgba(0,0,0,0.5)] bg-[#1a120b] px-4 md:px-8 pt-8 md:pt-12 pb-0 rounded-sm">
                {books.map((book) => (
                    <BookSpine
                        key={book.id}
                        book={book}
